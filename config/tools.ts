...
export const CALCULATORS: Calculator[] = [
  ...
  // FINANCE — Tax & Pay
  { slug:'tax-regime-compare',category:'finance',group:'Tax & Pay',icon:'⚖️',popular:true,title:'Old vs New Tax Regime Calculator',short:'Old vs New Regime',desc:'Compare tax under India\'s old and new regimes using your real income, 80C, HRA and other deductions.',metaDesc:'Free old vs new tax regime calculator India. See exactly which regime saves you more, with your real numbers.',tags:['old vs new tax regime','income tax india','80c','hra','tax saving'],tips:['The new regime has lower rates but almost no deductions — the old regime has higher rates but rewards 80C, HRA and other claims.','Re-check every year — if your deductions (especially HRA or a new home loan) change significantly, the better regime can flip.','You can choose a different regime each year if you have no business income — salaried employees aren\'t locked in.'],faq:[{q:'Which tax regime is better?',a:'It depends entirely on how much you can deduct. Roughly speaking, if your total 80C + HRA + other deductions exceed about ₹3.5-4L, the old regime usually wins; below that, the new regime\'s lower rates tend to win. This calculator gives your exact answer.'},{q:'Can I switch between old and new regime every year?',a:'Yes, if you\'re a salaried employee with no business income, you can choose either regime each financial year when filing your return.'},{q:'Is the new regime the default now?',a:'Yes — since FY 2023-24, the new regime is the default. You must actively opt for the old regime if you want to use it.'}],wflTopics:[{title:'How to save tax legally in India',slug:'tax-saving-guide-india'}],
    interpretation:'The "better" regime isn\'t fixed — it\'s a function of your deductions, and those change year to year (a new home loan, a lapsed insurance policy, a move to a no-HRA remote job). Re-run this calculator whenever your deduction picture changes materially, not just once when you first decide.',
    commonMistakes:[{mistake:'Assuming the new regime is always better because rates are lower.',fix:'Lower rates only help if you have few deductions to lose. Someone with a home loan, HRA, and full 80C usage can still come out ahead under the old regime despite its higher headline rates — run the actual numbers.'},{mistake:'Forgetting that 80C has a ₹1.5L cap regardless of how much you actually invest.',fix:'PPF + ELSS + EPF + life insurance premiums all share the same ₹1.5L combined limit under the old regime — investing ₹2L across these still only gets you ₹1.5L of deduction.'},{mistake:'Not re-evaluating after a life change (new home loan, job change, marriage).',fix:'Any of these can shift your deduction total enough to flip which regime wins. Treat this as an annual check, not a one-time decision.'}],
    pairWith:[
      { slug:'hra', reason:'HRA exemption only applies under the old regime — calculate your exact HRA exemption here, then plug it into the comparison above.' },
      { slug:'salary', reason:'See your full take-home breakdown, including PF and other deductions, alongside the regime comparison.' },
    ]},
  ...
  { slug:'take-home-salary',category:'finance',group:'Tax & Pay',icon:'💵',title:'Take-Home Salary Calculator',short:'Take-Home Salary',desc:'Estimate your monthly in-hand salary from your annual CTC.',metaDesc:'Free take-home salary calculator. Estimate your monthly in-hand pay from CTC.',tags:['take home salary','ctc','in-hand salary'],tips:['CTC includes employer contributions you never see in-hand (employer PF, gratuity) — always ask for the monthly in-hand figure when comparing offers.','A higher CTC with a lower basic-to-CTC ratio can sometimes mean a lower in-hand salary than expected — check the actual breakup.'],faq:[{q:'Why is my in-hand salary lower than CTC divided by 12?',a:'CTC includes employer PF contribution, gratuity, and other components that are part of your total cost to the company but are not paid to you monthly.'}],wflTopics:[{title:'What to do with your first salary',slug:'how-to-invest-first-salary'}],
    content:{
  ...
  { slug:'term-vs-ulip',category:'finance',group:'Insurance',icon:'⚖️',popular:true,title:'Term Insurance vs ULIP/Endowment Calculator',short:'Term vs ULIP',desc:'Compare "buy term, invest the rest" against a ULIP or endowment plan with the same premium.',metaDesc:'Free term insurance vs ULIP calculator. See which builds more wealth for the same premium, with real numbers.',tags:['term insurance vs ulip','term vs endowment','buy term invest rest','life insurance comparison'],tips:['Term insurance is pure protection — no maturity value — which is exactly why it\'s so much cheaper per rupee of cover than a ULIP or endowment plan.','ULIPs and endowment plans bundle insurance charges and fund management fees into one product, which historically drags net investment returns well below what a standalone equity fund delivers.','This comparison assumes real discipline to actually invest the premium difference — the "invest the rest" half only works if you follow through.'],faq:[{q:'Is buying term insurance and investing separately really better than a ULIP?',a:'For most people, yes — historically by a wide margin, because term insurance is dramatically cheaper for the same cover, and a standalone equity investment usually outperforms a ULIP\'s bundled, fee-heavy fund after charges.'},{q:'What is the one real advantage of a ULIP or endowment plan?',a:'Forced discipline — some people who would never invest on their own end up saving because the premium is contractually due. If that describes you, weigh that behavioural benefit against the numbers here.'},{q:'Are ULIP returns guaranteed?',a:'No — ULIPs are market-linked, same as any equity or debt fund, just with insurance and fund management charges layered on top that reduce net returns.'}],
    interpretation:'The "term + invest" side of this comparison only works out in practice if the premium difference is actually invested every year, not left in a savings account or spent. If you know you won\'t have that discipline, the gap shown here is the price of that behavioural gap, not a knock against the math.',
    commonMistakes:[{mistake:'Comparing only the sum assured or maturity value without factoring in the cost (premium) to get there.',fix:'A ULIP\'s maturity value looks attractive in isolation, but it\'s funded by a premium many times higher than term insurance for the same cover — always compare on a same-premium or same-outcome basis, which is what this calculator does.'},{mistake:'Surrendering a ULIP early without checking the exit penalty.',fix:'ULIPs typically have a 5-year lock-in and can carry surrender charges — if you\'re planning to switch to term + separate investing, check your specific policy\'s exit terms first rather than assuming a clean exit.'},{mistake:'Letting an insurance agent\'s commission incentive go unquestioned.',fix:'ULIPs and endowment plans typically pay agents significantly higher commissions than term insurance, which is part of why they\'re pushed harder — that\'s a reason to get independent numbers, not a reason to distrust every recommendation.'}],
    pairWith:[
      { slug:'term-insurance', reason:'Once you\'ve decided on term insurance, use the Human Life Value method here to find the right cover amount for your situation.' },
      { slug:'sip', reason:'Model the "invest the rest" side of this comparison in detail — step-up SIP, different return assumptions, and the full year-by-year growth.' },
    ]},
  ...
  // Health — Fitness
  { slug:'vo2max',category:'health',group:'Fitness',icon:'🫁',title:'VO2 Max Calculator',short:'VO2 Max',desc:'Estimate your VO2 max (aerobic fitness) without a treadmill test, using a validated non-exercise formula.',metaDesc:'Free VO2 max calculator. Estimate your aerobic fitness level without a lab test.',tags:['vo2 max','aerobic fitness','cardio fitness'],tips:['VO2 max naturally declines with age, but regular cardiovascular training can significantly slow this decline.'],faq:[{q:'What is a good VO2 max?',a:'It varies by age and sex, but generally above 45-50 mL/kg/min is considered good to excellent for most adults.'}],wflTopics:[{title:'Heart rate zones and training',slug:'heart-rate-zones-training'}],
    content:{
  ...
  { slug:'apy', category:'finance', group:'Grow Money',
    ...
    faq:[{q:'What is APY?',a:'Annual Percentage Yield — the real return including compound interest.'}],wflTopics:[{title:'Compound interest: the complete guide',slug:'compound-interest-complete-guide'}] },
  ...
  { slug:'cagr', category:'finance', group:'Grow Money',
    ...
    faq:[{q:'What is CAGR?',a:'Compound Annual Growth Rate — the steady annual rate at which an investment grew.'}],wflTopics:[{title:'Compound interest: the complete guide',slug:'compound-interest-complete-guide'}] },
  ...
  { slug:'future-value', category:'finance', group:'Grow Money',
    ...
    faq:[{q:'What is future value?',a:'The value of a current asset at a future date based on an assumed growth rate.'}],wflTopics:[{title:'Compound interest: the complete guide',slug:'compound-interest-complete-guide'}] },
  ...
  { slug:'irr', category:'finance', group:'Grow Money',
    ...
    faq:[{q:'What is IRR?',a:'The discount rate that makes NPV of all cashflows equal to zero. Higher is better.'}],wflTopics:[{title:'Index funds vs active funds in India',slug:'index-fund-vs-active-india'}] },
  ...
  { slug:'credit-card', category:'finance', group:'Borrow Money',
    ...
    faq:[{q:'Why does minimum payment take so long?',a:'Most minimum payment goes to interest, barely reducing the principal.'}],wflTopics:[{title:'The complete debt payoff guide',slug:'debt-payoff-complete-guide'}],
  ...
  { slug:'mortgage-refinance', category:'finance', group:'Borrow Money',
    ...
    faq:[{q:'When should I refinance?',a:'When the new rate saves enough to recover closing costs within 2 years.'}],wflTopics:[{title:'The complete home loan guide for India',slug:'home-loan-guide-india'}] },
  ...
  { slug:'pay-raise', category:'finance', group:'Tools',
    ...
    faq:[{q:'What is a good pay raise?',a:'6-8% covers inflation. 10-15% is a meaningful increase. 20%+ is excellent.'}],wflTopics:[{title:'How to save on an average salary',slug:'how-to-save-on-average-salary'}] },
  ...
  { slug:'salary-hourly', category:'finance', group:'Tools',
    ...
    faq:[{q:'How to convert annual salary to hourly?',a:'Divide annual salary by (hours per week × 52 weeks).'}],wflTopics:[{title:'What to do with your first salary',slug:'how-to-invest-first-salary'}] },
  ...
  { slug:'loan-payoff', category:'finance', group:'Borrow Money',
    ...
    faq:[{q:'How to pay off loan faster?',a:'Pay extra towards principal, make bi-weekly instead of monthly payments, or apply lump sums.'}],wflTopics:[{title:'The complete debt payoff guide',slug:'debt-payoff-complete-guide'}] },
];
